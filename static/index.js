...
// [PERF] Raised 50 → 100. The server keeps the whole history in a warm RAM
// index and serves any page in O(log n), so a bigger first page means fewer
// round trips on load and fewer pagination fetches while scrolling. Client
// render cost is amortised by content-visibility on off-screen bubbles.
const PAGE_SIZE      = 100;
// [PERF] DOM capping window. Raised 400 → 800: with per-message
// content-visibility + containment + has-media size hints + lazy media,
// ~800 retained nodes stay cheaper than churning nodes in/out every rapid
// scroll-back/forward. Tuned for smoothness, not minimal DOM count.
const DOM_CAP        = 800;
...
const EDIT_WINDOW_MS = 30 * 60 * 1000; // must mirror server.py's EDIT_WINDOW_SECONDS
...
// http://localhost/127.0.0.1 specifically. On the dedicated server this
// means clipboard support kicks in the moment a reverse proxy (Caddy/
// nginx) terminates TLS in front of us; if you're still reaching the box
// on plain http://<public-ip>:5000 for a smoke test, navigator.clipboard
// is undefined and the old code called .writeText straight off that
// undefined, which throws synchronously — before any Promise exists — so
// the chained .catch() never even attached. That's the actual bug: every
// click threw silently, nothing ever happened.
//
...
// [PERF] Conditional-request cache. The server etags every /history page;
// we keep the last body + etag per URL and send If-None-Match on repeat
// requests. An unchanged page comes back 304 (a few bytes) and we reuse the
// body we already parsed — no re-download, no re-parse. This is what makes
// scroll-back on an idle channel effectively free.
const _historyCache = new Map();   // url → { etag, body }

async function _fetchHistory(url, label) {
    try {
        const prior = _historyCache.get(url);
        const headers = prior && prior.etag ? { "If-None-Match": prior.etag } : {};
        const res = await fetch(url, { headers });

        if (res.status === 304 && prior) {
            return prior.body;                    // server says "unchanged"
        }
        if (!res.ok) throw new Error(`HTTP ${res.status}`);

        const body = await res.json();
        const etag = res.headers.get("ETag");
        _historyCache.set(url, { etag: etag || null, body });
        // bound the map so a long session doesn't grow it without limit
        if (_historyCache.size > 64) {
...
function buildMessageEl(msg, prevUser) {
    ...
    const shortTime = messageTime(msg.timestamp);
...
async function searchGiphy() {
    ...
    try {
        // [PERF][SEC] Giphy search now goes through the server's /api/giphy/search
        // proxy instead of hitting api.giphy.com directly from the browser:
        //   · same-origin request (no cross-origin TLS handshake per search),
        //   · the server holds the API key (it used to ship to every client),
        //   · the server slims each result down to {preview, full, title} —
        //     ~90% less JSON over the wire, and responses are gzip-compressed,
        //   · identical queries are cached server-side for 10 minutes.
        const response = await fetch(`/api/giphy/search?q=${encodeURIComponent(query)}&limit=18`);
        ...
        result.data.forEach(item => {
            const img = document.createElement("img");
            img.src = item.preview;                 // slim proxy payload: fixed_height_small
            img.loading = "lazy";
            ...
            img.addEventListener("mouseenter", () => img.style.transform = "scale(1.03)");
            img.addEventListener("mouseleave", () => img.style.transform = "scale(1.0)");
            img.addEventListener("click", () => stageRemoteGiphyAsFileObject(item.full));
            img.addEventListener("keydown", e => {
                if (e.key === "Enter" || e.key === " ") { e.preventDefault(); stageRemoteGiphyAsFileObject(item.full); }
            });
            grid.appendChild(img);
        });
