...
async function copyAsset(msgEl) {
    ...
    // ── IMAGES: try four paths, from best to acceptable ─────────────────
    if (kind === "image") {
        // 1. Authenticated fetch → ClipboardItem (best; a real bitmap on
        //    any OS). The blob is kept for step 3 so the legacy path never
        //    has to re-request the URL without session cookies.
        let blob = null;
        try {
            ...
            // 2. Re-encode to PNG and retry — some formats (webp, avif)
        ...
        // 3. Node-selection execCommand — works on http:// for same-origin
        ...
        // 4. Bitmap copy failed or was verified empty. Fall back to the
